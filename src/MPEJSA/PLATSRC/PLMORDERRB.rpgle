...
AR001   //---------------------------------------------------------
  ...
  |            write  pldltordr;
  |            delete plactordr;
  |            jobId = a_orodr# +'-'+ a_or#dsp +'/?id_type=external';
  |            cancelWFJob(jobId);
  |          endif;
  |        enddo;
  |
  ...
  |        ORDER_Aft = JeRcd;
  |
  |        //remove Orders that have changed for:
  |        if (b_orstat <> a_orstat And
  |        //Completed(Empty) Load
  |           (a_orstat = 'E') or
  |        //Voided Load with no prior pickup
  |           (a_orstat = 'A' and b_or#dsp <> a_or#dsp
  |            and a_or#dsp = '00' and b_ordsp# <> a_ordsp#
  |            and a_ordsp# = '00'))
AR002      //Voided Load with prior pickup
AR002      Or (b_orstat = a_orstat And (a_orstat = 'D'
AR002         and a_or#dsp > '00' and b_or#dsp <> a_or#dsp
AR002         and a_ordsp# = '00' and b_ordsp# <> a_ordsp#))
  |        //Terminated Load
  |        Or (b_orstat = a_orstat And (a_orstat = 'D'
  ...
  |        endif;
  |
  |        //Dispatched Order has been Cancelled.
  |        if b_orstat <> a_orstat And a_orstat = 'C';
  |
  |          //remove preplan order for all drivers.
  |          cancelPPjob(b_orodr#);
  |
  ...
  |            write  pldltordr;
  |            delete plactordr;
  |            jobId = b_orodr# +'-'+ b_or#dsp +'/?id_type=external';
  |            cancelWFJob(jobId);
  |          endif;
  |
  |        //Previously Active Order has been reset to Dispatched.
  |        elseif b_orstat <> a_orstat And a_orstat = 'D'
  |           And b_ordsp# = a_ordsp# And a_or#dsp > '00';
  |
  ...
  |          endif;
  |
  |        //Previously Dispatched Order has been reset to Active.
  |        elseif b_orstat <> a_orstat And b_orstat = 'D'
  |           And a_orstat = 'A'
  |           And b_ordsp# = a_ordsp# And a_or#dsp > '00';
  |
  |          //remove preplan order for all drivers.
  ...
  |          enddo;
  |
AR003      //Terminated Load FAILED (Order dispatched/driver available)
  |        elseif (b_orstat = a_orstat And (a_orstat = 'D'
  |           and a_or#dsp > '00' and b_or#dsp = a_or#dsp
  |           and a_ordsp# > '00' and b_ordsp# = a_ordsp#));
  |          chain (a_orodr#:a_or#dsp) plactordp;
  |          if %found(plactordp);

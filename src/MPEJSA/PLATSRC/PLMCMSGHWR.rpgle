      ...
      *  03/26/20   R008  JB/PS  Ensure truck is active for workflow.
      *  04/03/20   R009  JB/PS  Accomodate teams for Cancel Preplan.
      *  04/13/20   R010  JB/PS  Removed WF Cancel in legacy routine.
      *  05/08/20   R011  JB/PS  Trigger workflow_job update from T48.
      *****************************************************************
...
AR007D ffmsg           s               n   inz(*off)
AR007D wfmsg           s               n   inz(*on)
AR001d w_time4         s              4a
      *----------------------------------------------------------
      * Entry point
      *----------------------------------------------------------
...
AR007   Begsr MCMSGHWFsr;
  ...
  |          or a_mhpmid = 'T43';
  |           chain a_mhunit units;
AR008         if %found(units) And a_mhpmid = 'T40' And unstat = 'D'
AR008         OR %found(units) And a_mhpmid <> 'T40';
DR008         //if %found(units) = *on;
  |             chain undr1 plactdrvp;
...
AR006      //execute for new fuel stops on active drivers only.
 ...
 |          //resend workflow if current T00 is EF for current disp.
  ...
  |                passdate = T40date;
  |                passtime = T40time;
DR010              //jobId = unord# +'-'+ undisp
DR010              //        +'/?id_type=external';
DR010              //cancelJob(jobId);
  |                createjob(a_mhunit);
  |              endif;
  |            endif;
  |          endif;
...
AX001    //Sendload(unit#:mode:Pplan:order#:disp#:ldmil:mtmil);
  ...
  |        exsr getT40time;
 |        //resend workflow for current disp from T48.
  |        if t40time > *zero And (a_mhdate > T40date or
  |          a_mhdate = T40date and a_mhtime >= T40time);
  |          passdate = T40date;
  |          passtime = T40time;
...
AR009      if a_mhdir = 'O';
  ...
  |            m +=1;
  |          enddo;
  |          order# = %subst(messageA(2):10:7);
  |          disp#  = %subst(messageA(2):26:2);
  |          chain (order#:disp#) plactordp;
  |          if %found(plactordp);
  |            cnord# = plactord;
  |            cndisp = plactdisp;
CR005          cncode = 'V';
...
AR009      if a_mhdir = 'O';
  ...
  |            m +=1;
  |          enddo;
  |          order# = %subst(messageA(3):9:7);
AR009        cancelPPjob(order#);
             //changed to write to a transaction processing file
DR009        //PLWFORD  = a_mhord#;
AR009        PLWFORD  = order#;
             PLWFTYPE = 'PP';
